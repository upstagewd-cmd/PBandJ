...
import { ArrowLeft, Trophy, Target, TrendingUp, Star, LogOut, User } from "lucide-react";

export default function ProfilePage() {
  const [, setLocation] = useLocation();
  const { user } = useUser();
  const { signOut } = useClerk();
  const { data: profile, isLoading } = useGetMyProfile({ query: { retry: false, queryKey: ["myProfile"] } });
            ...
            {profile.matchesPlayed === 0 && (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-sm mt-1">
                  Join a tournament and make sure you're signed in when you enter your name.
                </p>
              </div>
            )}
          </>
