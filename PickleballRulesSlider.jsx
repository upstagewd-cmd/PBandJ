...
const lineColor = "rgba(233,231,225,0.28)";

/* ---------- shared court diagram pieces ---------- */

function CourtShell({ children, viewBox = "0 0 200 300", maxHeight = 250 }) {
  return (
...
function Dot({ x, y, r = 6, fill = T.white, label, labelColor = T.black }) {
  return (
    <g>
    ...
    <div className="flex flex-wrap gap-x-4 gap-y-1.5 mt-3">
      {items.map((it, i) => (
        <div key={i} className="flex items-center gap-1.5">
          ...
          <span className="text-[11px]" style={{ color: bodyColor }}>{it.text}</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="flex items-center gap-2">
        {tiles.map((n, i) => (
          <React.Fragment key={i}>
            ...
            {i < tiles.length - 1 && <span style={{ color: bodyColor }}>–</span>}
          </React.Fragment>
        ))}
      </div>
      <div className="flex gap-4">
        {labels.map((l, i) => (
          <span key={i} className="text-[10px] uppercase tracking-wide" style={{ color: bodyColor, fontFamily: "'JetBrains Mono', monospace" }}>{l}</span>
        ))}
      </div>
    </div>
  );
          ...
          <div className="flex flex-col items-center gap-1.5">
            <div
              ...
              style={{
                ...
                minWidth: 62,
                background: T.black,
                border: `1px solid ${i === 2 ? T.lime : "rgba(233,231,225,0.25)"}`,
                color: i === 2 ? T.lime : T.white,
            ...
            <span className="text-[9px] uppercase tracking-wide" style={{ color: i === 2 ? T.lime : bodyColor, fontFamily: "'JetBrains Mono', monospace" }}>{st.tag}</span>
          </div>
          {i < steps.length - 1 && <ChevronRight size={16} color={T.stone} style={{ opacity: 0.5, marginTop: -14 }} />}
        </React.Fragment>
      ))}
    </div>
  );
  ...
  const Mini = ({ label, players }) => (
    <div className="flex flex-col items-center gap-1.5 flex-1">
      <svg viewBox="0 0 120 180" style={{ width: "100%", height: "auto", maxHeight: 170 }}>
        ...
        {players.map((p, i) => (
          <circle key={i} cx={p[0]} cy={p[1]} r="5" fill={T.white} />
        ))}
      </svg>
...
export default function PickleballRulesSlider() {
  ...
  return (
    ...
    >
      ...
      {/* Sized for iPhone (390-430pt) and flagship Samsung (360-412dp) viewports */}
      <div
        className="relative w-full mx-auto rounded-[28px] overflow-hidden flex flex-col"
        style={{
          maxWidth: 402,
          minHeight: 760,
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-2">
          ...
          <span
            className="pb-display text-[11px] uppercase px-2.5 py-1 rounded-full"
        ...
        <div key={index} className="pb-card-enter px-5 pt-5 pb-3 flex-1 overflow-y-auto">
          {slide.kind === "intro" && (
            <div className="flex flex-col gap-4">
              <div className="w-40 mx-auto">{slide.visual}</div>
          ...
          {slide.kind === "recap" && (
            <div className="flex flex-col gap-4">
              <div className="w-36 mx-auto">{slide.visual}</div>
              <div className="flex flex-col gap-2.5">
                <span className="pb-mono text-xs uppercase tracking-widest text-center" style={{ color: T.lime }}>{slide.eyebrow}</span>
                <h1 className="pb-display text-2xl leading-tight text-center" style={{ color: T.white, fontWeight: 600 }}>{slide.title}</h1>
                <ol className="flex flex-col gap-2 mt-1">
                  {slide.recap.map((item, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <span className="pb-mono text-xs shrink-0 mt-0.5" style={{ color: T.lime, width: 16 }}>{i + 1}</span>
          ...
          {!slide.kind && (
            <div className="flex flex-col gap-3.5">
              <div className="w-44 mx-auto">{slide.visual}</div>
              ...
              <div className="flex flex-col gap-3.5">
                  ...
                  <ul className="flex flex-col gap-2 mt-1">
                    {slide.list.map((item, i) => (
                      <li key={i} className="flex items-start gap-2.5">
                        <span className="rounded-full shrink-0" style={{ width: 5, height: 5, marginTop: 8, background: T.lime }} />
                        <span className="text-[13.5px] leading-relaxed" style={{ color: T.stone }}>{item}</span>
                      </li>
                    ))}
                  </ul>
                ...
                {slide.footnote && (
                  <p className="text-[12.5px] leading-relaxed pt-2 mt-1" style={{ color: T.lime, borderTop: `1px dashed rgba(183,227,52,0.3)` }}>
                    {slide.footnote}
                  </p>
                )}
              </div>
        ...
        <div className="flex items-center justify-between px-5 pb-6 pt-3 shrink-0">
          <button
            onClick={prev}
          ...
          <div className="flex items-center gap-1.5">
            {SLIDES.map((_, i) => (
              <button
                ...
                style={{ width: i === index ? 18 : 6, height: 6, background: i === index ? T.lime : "rgba(233,231,225,0.25)" }}
              />
            ))}
          </div>
